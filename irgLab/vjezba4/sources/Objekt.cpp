...
void Objekt::IscrtajModel(Shader& sjencar) {
	//cout << "Da kolega ja printam model" << endl;
	for (int i = 0; i < meshVektor.size(); i++) {
		meshVektor[i].Iscrtaj(sjencar);
	}
}

void Objekt::updateTransMatriceMeshevima(glm::mat4 matricaTransformacije) {
	for (int i = 0; i < meshVektor.size(); i++) {
		meshVektor[i].transMatricaUpdate(matricaTransformacije);
	}
}

void Objekt::transformirajMesheve() {
	for (int i = 0; i < meshVektor.size(); i++) {
		meshVektor[i].transformiraj();
	}
}
