...
public:
	...
	Objekt(string path, string resPath);
	...
	void assimpUcitaj(string path, string resPath);


	//idemo po root nodeu i njegovoj djeci i ucitavamo sve podatke o meshevima
	void razrijesiNode(const aiScene* scene, aiNode* node, string resPath);

	//funkcija koja pretvara aiMesh u Mesh objekt i vraca ga za daljnu obradu
	Mesh pretvaracMesh(const aiScene* scene, aiMesh* AssimpMesh, string resPath);
