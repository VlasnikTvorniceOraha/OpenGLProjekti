...
void Shader::setUniformMat4(const std::string& name, glm::mat4 matrica)const {

	glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &matrica[0][0]);
}

void Shader::setUniformVec3(const std::string& name, glm::vec3 vektor)const {

	glUniform3fv(glGetUniformLocation(ID, name.c_str()), 1, &vektor[0]);
}
