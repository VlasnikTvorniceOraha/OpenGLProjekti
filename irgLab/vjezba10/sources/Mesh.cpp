...
Mesh::Mesh(std::vector<Vertex> vrhovi, std::vector<unsigned int> indeksi, vector<Texture> teksture)
{
	this->vrhovi = vrhovi;
...
void Mesh::ucitajBuffere() {
	...
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, TexKordinate));
...
void Mesh::Iscrtaj(Shader& sjencar, bool Culling, glm::mat4 Offset, Material material) {
	...
	glBindTexture(GL_TEXTURE_2D, teksture[1].id);
...
void Mesh::normaliziraj() {
	...
	for (int i = 0; i < vrhovi.size(); i++) {
		...
		noviVrh.TexKordinate.x = vrhovi[i].TexKordinate.x;
		noviVrh.TexKordinate.y = vrhovi[i].TexKordinate.y;
