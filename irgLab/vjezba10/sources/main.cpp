...
int main(int argc, char* argv[]) {
	...
	Objekt ucitanaKocka(objPath, resPath);
	Objekt ucitanaKocka2(objPath, resPath);
	...
	*/
	...
	glm::mat4 offset = TransObjekt.Translacija(glm::vec3(-1.0f, -1.0f, -3.5f));
	...
	//vrijeme je za sjencano mapiranje

	//stvaranje frame buffera

	unsigned int dubinaFrameBuffer;
	glGenFramebuffers(1, &dubinaFrameBuffer);

	unsigned int dubokaMapa;
	glGenTextures(1, &dubokaMapa);

	//samo koristiti dubinu i format depth component
	...
	//stavi to u depth buffer

	glBindFramebuffer(GL_FRAMEBUFFER, dubinaFrameBuffer);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, dubokaMapa, 0);
	//Nema boje pa je GL_NONE
	...
	while (glfwWindowShouldClose(window) == false) {
		...
		//epski prvi pass za shadow mapping
		...
		ucitanaKocka.IscrtajModel(*sjencar[0], 1, offset);
